...
accel_clean =
  ...
  select(day_id, day, week, act_mins, act_count)
accel_clean
```
```
**Description:**  
...
accel_total = 
  accel_clean %>% 
  group_by(day_id) %>% 
  summarize(act_total_n = sum(as.numeric(act_count)))
kable(accel_total)

accel_total_plot = 
  accel_total %>%
  ggplot(aes(x = day_id, y = act_total_n)) + 
  geom_line()
accel_total_plot
```
...
accel_total_plot = 
  accel_clean %>% 
  separate(act_mins, c("act", "mins"), sep = "_") %>% 
  select(-act) %>% 
  mutate(mins = as.numeric(mins)) %>% 
  mutate(act_count = as.numeric(act_count)) %>% 
#accel_total_plot
  ggplot(aes(x = as.numeric(mins), y = act_count, color = day_id)) + 
    geom_line()
accel_total_plot
```
